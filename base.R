...
file_size <- "1e8"
file_name <- here::here("data", paste0("measurements.", file_size, ".csv"))
...
res <- bench::mark(
  ...
  read_delim_tsapply = {
    print("read_delim_tsapply")
    df <- read.delim(file_name, sep = ",", header = TRUE)
    t(sapply(split(df$measurement, df$state), function(x) {
